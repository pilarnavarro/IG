...
void Camara3Modos::desplRotarXY( const float da, const float db )
{
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         org_polares[0]=org_polares[0]-da*0.1;
         org_polares[1]=org_polares[1]+db*0.1;

         if(org_polares[1]<-(M_PI/2-0.001)) org_polares[1]=-(M_PI/2-0.001);
	
	      else if(org_polares[1]>(M_PI/2-0.001)) org_polares[1]=(M_PI/2-0.001);
      ...
      case ModoCam::prim_pers_rot :
      {
         ...
         org_polares[0]=org_polares[0]-da*0.01;  //*0.1 para que la cámara no de saltos grandes
         org_polares[1]=org_polares[1]+db*0.01;  //Sumamos o restamos para que la cam se mueva en correspondencia
                                                //con las flechas del teclado                        

         //El tope en el eje Y esta en -pi/2 y pi/2 
         //Al llegar a los límites hace cosas raras por eso quitamos 0.001 para que no llegue al tope
         if(org_polares[1]<-(M_PI/2-0.001)) org_polares[1]=-(M_PI/2-0.001); 
	
	      else if(org_polares[1]>(M_PI/2-0.001)) org_polares[1]=(M_PI/2-0.001);

         Tupla3f n=org_cartesianas;
         org_cartesianas=Cartesianas(org_polares);
         punto_atencion=punto_atencion - (org_cartesianas-n);
      ...
      case ModoCam::prim_pers_despl :
      {
         ...
         //No cambian las coordenadas, no hace falta hacer esto
         //org_cartesianas=org_cartesianas-da*eje[0]+db*eje[1];
         //org_polares=Esfericas(org_cartesianas);
...
void Camara3Modos::moverZ( const float dz )
{
   ...
   switch( modo_actual )
   {
      case ModoCam::examinar :
      {
         ...
         float rmin=1.0;   //Valor minimo del radio, distancia de la camara al punto de atencion

         org_polares[2]=rmin + (org_polares[2]-rmin)*pow(1.1,dz);
         org_cartesianas=Cartesianas(org_polares); 
      ...
      case ModoCam::prim_pers_despl :
      {
         ...
         //org_cartesianas=org_cartesianas+dz*eje[2];  
         //org_polares=Esfericas(org_cartesianas);

...
for(int i=0;i<vpf.size();i++){
   ...
   Tupla4f pos={cos((vpf[i]->longi)*M_PI/180)*cos((vpf[i]->lati)*M_PI/180),
   sin((vpf[i]->lati)*M_PI/180),sin((vpf[i]->longi)*M_PI/180)*cos((vpf[i]->lati)*M_PI/180),0.0};
   pos.normalized();
   vec_dir.push_back(pos);
